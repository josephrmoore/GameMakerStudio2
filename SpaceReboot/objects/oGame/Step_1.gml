...
if(screen_state == SCREENSTATE.PAUSED){
	...
	if(key_shoot_pressed || key_missile_pressed || key_slash){
		var active_mods = 0;
		for(var i=0; i<ds_list_size(player_mods_activated); i++){
			if(ds_list_find_value(player_mods_activated, i)){
				active_mods++;
			}
		}
		if(ds_list_find_value(player_mods, mod_screen_selected)){
			if(ds_list_find_value(player_mods_activated, mod_screen_selected)){
				// deactivate this one
				ds_list_replace(player_mods_activated, mod_screen_selected, false);
				access_player_stats("load");
			} else if(active_mods < 4){
				// activate this one
				ds_list_replace(player_mods_activated, mod_screen_selected, true);
				access_player_stats("load");
			}
		}
	}
	...
	if(key_down){
		map_y_offset = clamp(map_y_offset-2, -878, 0);
	}
	if(key_up){
		map_y_offset = clamp(map_y_offset+2, -878, 0);
	}
